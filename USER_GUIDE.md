...
- **Chart patterns library and Patterns tab** — 75 chart, candlestick and price-action patterns in 7 categories, forming or confirmed, with neckline, target, stop hint and quality score; a full `Catalog` with the app's own EGX track record.
...
|---|---|---|
| **Trend** | Price is above the 50-day average and the 20-day average is above the 50-day (if the recent swings are still making lower highs and lower lows it drops to `⚠` — "the trend is tiring") | Price and the 20-day are both below the 50-day — a downtrend |
...
**What it is** — A one-line strip of world markets fetched from Yahoo: US indices, key currencies, ETFs and crypto, each with its latest value and percentage change.

**What you see** — Up to 14 items in a row. Each item shows the market's **name**, its **value**, and its **% change** coloured green (up), red (down) or grey (flat). A `—` means that value was unavailable. If the data arrives in an unexpected shape it is shown as a plain list instead of a strip. On failure the card shows an error box beginning `Global snapshot: …`.
...
|---|---|---|---|
| **Trend** | Price is above the 50-day average and the 20-day average is above the 50-day (the text adds `50-day rising` or `50-day still flat`, and whether swings make `higher highs and higher lows`). | The averages line up but the recent swings are lower highs and lower lows (`the trend is tiring`), or the picture is `Mixed: … No clear trend yet.` | Price and the 20-day average are both below the 50-day — `a downtrend. Buying against it needs a reason.` |
...
| **Price action** | Confirmed bullish events on the recent sessions outnumber bearish ones (names listed). | `No decisive price-action event on the last sessions.` | Any bearish reversal signal on the tape — bull trap, false breakout, change of character, failed retest or upthrust — or bearish events simply outnumber bullish. |
...
**What it is** — where analysis becomes an executable plan: entry, stop, two targets, the risk-to-reward ratio, a quality grade, the alternative scenario, and EGX-specific warnings about whether the plan is realistic. The levels are drawn on the chart as `entry`, `stop`, `T1`, `T2`.
...
- A grey footer: `Tested zones (2+ touches) are drawn on the chart. Zones = clusters of swing highs/lows (window 3) within 0.75 ATR over the last year, strength = touches + recency + volume at the touches; plus 52-week extremes, round numbers near price, SMA50/SMA200.`
...
- A grey **count line**, e.g. `2 chart shapes · 1 price-action event · 3 candlesticks`.
- A **compact list**, ordered: chart shapes first (reversals, triangles, wedges, channels, continuations), then price-action events, then candlesticks; within each group `confirmed` before `forming`, then by quality. **Only the first six are shown**; a `Show all 11` button reveals the rest.
- Each item is a collapsed row you can click to expand. The **collapsed row** shows:

| Part | Example | Meaning |
|---|---|---|
| Name | `Double bottom`, `Ascending Triangle ↑ broke up`, `Bullish Engulfing` | The pattern; trendline shapes add `↑ broke up` / `↓ broke down` once broken. |
...
- The **expanded body** shows:
  - A grey line: `reversal · reversal · reliability high · common` — the category, kind, and two qualitative tiers from the classic pattern references: **reliability** (`high` / `medium` / `low`) and **frequency** (`common` / `uncommon` / `rare`).
...
|---|---|---|---|
| **Trend** | Price above the 50-day average and the 20-day above the 50-day (ideally with higher highs and higher lows in the swings) | Mixed averages, or an uptrend whose recent swings are already making lower highs and lower lows ("the trend is tiring") | Price below the 50-day and the 20-day below the 50-day — a downtrend |
...
| **Price action** | Recent confirmed bullish events (price-action events and candlestick patterns) outnumber bearish ones | No decisive event | A red-flag reversal on the tape (bull trap, false breakout, change of character, failed retest, upthrust), or bearish events outnumber bullish |
...
- A header line: `N hits stored · M graded · K waiting for price history · weights change after 20 graded hits`, and a `Grade now` button.
...
| `Weight` | The scanner's multiplier in the `Candidates` ranking, as a badge. **Amber** = neutral 1.0 because fewer than 20 hits are graded at 10 days (hover to read e.g. "neutral 1.0 — 12/20 graded hits at 10d"). **Green** = above 1.0 (the scanner beats the index more often than not). **Red** = below 1.0. **Teal** = exactly 1.0 with a full sample (a 50% beat rate). Hover for the basis, e.g. "beat rate 62% over 34 hits at 10d". The formula is 2 × the 10-day beat rate, clamped between 0.5 and 1.5 |
| `n 5d` · `Beat EGX30 5d %` · `Avg excess 5d %` | How many hits have reached 5 sessions, the share of those that beat the index, and their average return above the index (green or red) |
| `n 10d` · `Win 10d %` · `Beat EGX30 10d %` · `Avg excess 10d %` | The same at 10 sessions, plus `Win 10d %` — the share of hits where the stock was simply *up* 10 sessions later |
| `n 20d` · `Beat EGX30 20d %` · `Avg excess 20d %` | The same at 20 sessions |

- Under the table, the basis: *Each scanner hit is graded by the stock's close-to-close return 5/10/20 sessions after the hit date, and by the excess over EGX30 for the same window. A scanner's weight in the Candidates ranking = 2 × its 10-day beat rate, clamped to [0.5, 1.5], and only after 20 graded hits.* and an expandable `Full scorecard payload`.
- Errors show as `Scorecard failed: …` or `Grading failed: …`.

Where the hits come from: the post-close job (every trading day at 15:00) runs the Candidates scan and stores one hit per scanner per stock, and the pattern scan stores confirmed, directional patterns of quality 50 or more. A live `Refresh` on the `Candidates` tab does **not** add hits — only the scheduled post-close run does, so the track record is not polluted by you pressing buttons. (The `Scan now` button on the `Patterns` tab, by contrast, does store its confirmed patterns as hits.) Each grading run takes the oldest ungraded hits first, up to 120 at a time; a hit whose 20-day outcome is still open is re-graded on later days (at most once a day) until it completes. The weekly maintenance job (Saturday 12:00) removes hits older than 180 days, so the record is a rolling six months, not all of history.
...
Two special reasons can appear on any row: `No price available for this symbol (Yahoo and snapshots both empty) — verdict cannot be computed; check the position manually.` (the row is shown as HOLD because nothing can be judged), and `Daily history unavailable (…) — trailing-stop and time-stop checks skipped.` when the candle download failed.
